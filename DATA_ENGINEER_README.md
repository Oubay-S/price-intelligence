...
Five Jupyter notebooks covering the full exploratory analysis lifecycle:

| Notebook | Focus |
|---|---|
| `01_data_understanding.ipynb` | Dataset overview, shape, data types, missing values |
| `02_data_cleaning.ipynb` | Null handling, price parsing, deduplication |
| `03_exploratory_analysis.ipynb` | Price distributions, category breakdowns, store comparisons |
| `04_statistical_tests.ipynb` | Hypothesis testing, price variance analysis |
| `05_final_insights.ipynb` | Business insights, final visualizations |

Results are exported and uploaded to BigQuery via `upload_analysis_to_bigquery.py` for access by the dashboard.

---

## 🐳 Docker Infrastructure (Data Layer)

| Container | Image | Role |
|---|---|---|
| `airflow-postgres` | `postgres:15-alpine` | Airflow metadata database |
| `airflow-init` | `price-intel-airflow` | One-off DB migration + admin user creation |
| `airflow-webserver` | `price-intel-airflow` | Airflow UI (`:8080`) |
| `airflow-scheduler` | `price-intel-airflow` | DAG scheduling engine |
| `nifi` | `price-intelligence-nifi` | Real-time JSON → Bigtable ingestion (`:8443`) |
| `dbt` | `dbt-labs/dbt-bigquery:1.8.2` | SQL transformations (run manually or via Airflow) |

All data-layer containers share the `price-intel-network` bridge network.

**GCP credentials** (`gcp-credentials.json`) are mounted read-only into:
- Airflow at `/opt/airflow/gcp-credentials.json`
- NiFi at `/opt/nifi/gcp-credentials.json`
- Backend at `/opt/gcp/gcp-credentials.json`
...
```env
AIRFLOW_UID=1000
GOOGLE_CLOUD_PROJECT=price-intelligence-495411
...
The CI pipeline runs on every push/PR to `develop` or `main`:

| Job | What it validates |
|---|---|
| `lint` | Flake8 Python errors + DAG syntax check |
| `pipeline-tests` | pytest unit tests under `tests/` |
| `dbt-validate` | `dbt deps` + `dbt parse` with a dummy GCP key |
| `integration-tests` | Full Docker Compose stack startup + DAG detection |
| `sast` | Bandit security scan (blocks on HIGH severity) |
| `secrets-scan` | TruffleHog + Gitleaks (blocks on verified secrets) |
| `dependency-scan` | pip-audit CVE scan on all requirements.txt |
| `docker-security` | Trivy image scan (Airflow, Backend, Frontend) |
| `iac-scan` | Terraform validate + Checkov + Hadolint |
...
├── airflow/
│   ├── dags/
│   │   └── price_intelligence_dag.py     ← Main Airflow DAG (full pipeline)
│   └── requirements.txt                  ← Airflow pip extras
├── scrapers/
│   ├── jumia/                            ← Jumia scraper + raw data
│   ├── sport-direct/                     ← Sport Direct scraper + raw data
│   ├── ebay/                             ← eBay scraper + raw data
│   ├── nifi_to_bigtable.py               ← NiFi ingestion script → Bigtable
│   ├── bigtable_to_bigquery.py           ← Export Bigtable → BigQuery
│   ├── load_all_to_bigtable.py           ← Bulk load historical CSVs → Bigtable
│   ├── product_quality.py                ← Product filtering rules library
│   ├── backfill_unknown_categories.py    ← Re-classify unknown categories in BQ
│   ├── bigquery_product_cleanup.py       ← Remove junk rows from BigQuery
│   └── cleanup_irrelevant_bigquery_products.py
├── dbt/
│   ├── models/
│   │   ├── staging/stg_prices.sql        ← Staging model
│   │   ├── intermediate/int_price_daily.sql ← Daily aggregation
│   │   └── mart/mart_price_trends.sql    ← Final mart for frontend
...
├── data-analysis/
│   ├── notebooks/                        ← 5 EDA notebooks
│   ├── run_eda_pipeline.py               ← Orchestrates notebook execution
│   ├── upload_analysis_to_bigquery.py    ← Pushes EDA results to BQ
│   └── export_for_dashboard.py           ← Dashboard data export
├── .env                                  ← GCP project + instance IDs
